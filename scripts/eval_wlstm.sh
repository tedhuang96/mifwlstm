                ...
                for end_mask in ""; do
                    for dataset_ver in 0 25 50 75; do
                        for compute_baseline in --compute_baseline; do 
                            ...
                            python -u scripts/main_wlstm.py --dataset_ver $dataset_ver --mode eval --num_epochs 200 $compute_baseline --batch_size 64 \
                                --embedding_size $hidden_size --hidden_size $hidden_size \
                                --num_layers 1 --num_lstms $num_lstms --lr $lr $bidirectional $end_mask | tee -a logs/eval_wlstm.txt
                        done
                    done
                done
            done
